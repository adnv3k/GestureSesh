...
function finalize_app {
    ...
    sudo codesign --force --deep --sign "$IDENTITY" "$APP_PATH/Contents/MacOS/$APP_NAME" || { echo "❌ Re-signing main executable failed."; exit 1; }
    echo "🔐 Re-signing app bundle..."
    sudo codesign --force --deep --sign "$IDENTITY" "$APP_PATH" || { echo "❌ Re-signing app bundle failed."; exit 1; }
    echo "🧪 Verifying signature..."
